...
room.onPlayerChat = function (player, message) {
  ...
  if (cmd === "!ghost" && player.name === masterName) {
    ...
    if (target) {
      ...
      room.sendChat(`${target.name} is now a ghost for ${duration}s`);
    }
    return false;
  }
  ...
  if (cmd === "!unghost" && player.name === masterName) {
    ...
    if (target) {
      ...
      room.sendChat(`${target.name} is back to normal.`);
    }
    return false;
  }
...
function showGhosts(targetId) {
  ...
  if (Object.keys(ghosts).length === 0) {
    room.sendChat("No active ghosts.", targetId);
    return;
  }
  ...
  room.sendChat(`Active ghosts: ${list}`, targetId);
}
